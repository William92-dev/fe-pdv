...
@ViewScoped
@Named("receptarFacturaMailParametroBean")
public class ReceptarFacturaMailParametroBean extends BaseBean implements Serializable {

	public static final Logger log = Logger.getLogger(ReceptarFacturaMailParametroBean.class);
	...
	private com.gizlocorp.gnvoice.modelo.FacturaRecepcion tranformaGuardaFactura(String claveAccesop, Date fechcaTMP){
		...
		try{
			...
			if(offline){
				...
				DateFormat dateFormat = new SimpleDateFormat("dd/MM/YYYY");
				String strDate = dateFormat.format(date);
				comprobante.append(autorizacionOfflineXML.getComprobante().split("<fechaEmision>")[0]);
				comprobante.append("<fechaEmision>");
				comprobante.append(strDate);
				comprobante.append("</fechaEmision>");
				comprobante.append(autorizacionOfflineXML.getComprobante().split("</fechaEmision>")[1]);
				
				autorizacionOfflineXML.setComprobante(comprobante.toString());
...
//				facturaObj.set
				...
				rutaArchivoXml = DocumentUtil.createDocument(getFacturaXML(autorizacionXML),
						FechaUtil.formatearFecha(now.getTime(),"ddMMyyyy"),
						facturaXML.getInfoTributaria().getClaveAcceso(), subFolder, "recibidoReim");
				facturaObj.setArchivo(rutaArchivoXml);
//				String rutaArchivoPdf = ReporteUtil.generarReporte(Constantes.REP_FACTURA,
		...
		while (rows.hasNext()) {
			...
			while (cells.hasNext()) {
				...
				if(cell.getColumnIndex()== index){
					...
					if(!strDate.equals(String.valueOf(cell.getColumnIndex()).substring(0,8))){
