...
import com.gizlocorp.gnvoice.json.model.SubsidioNotaCredito;
...
public class ReporteUtil {
	...
	private static String fechaFolder(String fechaEmision) {
	...
	// METODO RIDE FACTURA
	public static String generarReporte(String urlReporte, FacturaReporte fact,
		...
		log.info("***iniciando estructura pdf***"+urlReporte+"***"+dirServidor);
		...
		log.info("***iniciando estructura pdf 222 generarReporte***");
		...
		log.info("***iniciando estructura pdf 321 generarReporte***"+outputFile.toString());
		return outputFile.toString();

	}

	public static String generarReporte(String urlReporte,
			NotaDebitoReporte rep, String numAut, String fechaAut,
		...
		outputFile.append(rep.getNotaDebito().getInfoTributaria()
				.getClaveAcceso());
	...
	public static String generarReporteDocuClass(String urlReporte,
			NotaCreditoReporte rep, String numAut, String fechaAut,
			String fechaEmision, String subcarpeta, boolean rechazado,
			String logo, String dirServidor) throws Exception {
		...
		FileInputStream is = new FileInputStream("/data" + urlReporte);
		JRDataSource dataSource = new JRBeanCollectionDataSource(
				rep.getDetallesAdiciones());
				...
				.fillReport(
						...
						obtenerMapaParametrosReportes(
								obtenerParametrosInfoTriobutaria(rep
										...
										numAut, fechaAut, rechazado, logo,
										"/data"),
								obtenerInfoNC(rep.getNotaCredito()
										.getInfoNotaCredito(), rep)),
						dataSource);

		StringBuilder outputFile = new StringBuilder();
		outputFile.append(dirServidor);
	...
	public static String generarReporte(String urlReporte,
			GuiaRemisionReporte rep, String numAut, String fechaAut,
			GuiaRemision guiaRemision, String fechaEmision, String subcarpeta,
		...
		JasperPrint print = JasperFillManager.fillReport(
				is,
				obtenerMapaParametrosReportes(
						obtenerParametrosInfoTriobutaria(rep.getGuiaRemision()
								.getInfoTributaria(), numAut, fechaAut,
								rechazado, logo, dirServidor),
	...
	public static String generarReporteRetencion(String urlReporte,
		...
		DocumentUtil.createDirectory(outputFile.toString());
		outputFile.append("/");
		outputFile.append(rep.getComprobanteRetencion().getInfoTributaria().getClaveAcceso());
		outputFile.append(".pdf");
		JasperExportManager.exportReportToPdfFile(print, outputFile.toString());
		return outputFile.toString();
	}
	...
	private static Map<String, Object> obtenerMapaParametrosReportes(
			Map<String, Object> mapa1, Map<String, Object> mapa2) {
		mapa1.putAll(mapa2);
		log.info("***iniciando estructura pdf 222 obtenerMapaParametrosReportes ***");
		return mapa1;
	}

	private static Map<String, Object> obtenerParametrosInfoTriobutaria(
			InfoTributaria infoTributaria, String numAut, String fechaAut,
			boolean rechazado, String logo, String dirServidor)
			throws SQLException, ClassNotFoundException {

		log.info("***obtenerParametrosInfoTriobutaria***");
		Map<String, Object> param = new HashMap<String, Object>();
		param.put("RUC", infoTributaria.getRuc());
		log.info("***obtenerParametrosInfoTriobutaria 0***");
		param.put("CLAVE_ACC", infoTributaria.getClaveAcceso());
		log.info("***obtenerParametrosInfoTriobutaria 1***");
		param.put("RAZON_SOCIAL",
				StringEscapeUtils.unescapeXml(infoTributaria.getRazonSocial()));
		log.info("***obtenerParametrosInfoTriobutaria 2***");
		param.put("DIR_MATRIZ",
				StringEscapeUtils.unescapeXml(infoTributaria.getDirMatriz()));
		log.info("***obtenerParametrosInfoTriobutaria 3***");
		try {
			param.put("LOGO", new FileInputStream(logo));
			log.info("***obtenerParametrosInfoTriobutaria 4***");
		} catch (Exception ex) {
		...
		log.info("***obtenerParametrosInfoTriobutaria 5***");
		param.put("TIPO_EMISION", obtenerTipoEmision(infoTributaria));
		log.info("***obtenerParametrosInfoTriobutaria 6***");
		param.put("NUM_AUT", numAut);
		log.info("***obtenerParametrosInfoTriobutaria 7***");
		param.put("FECHA_AUT", fechaAut);
		log.info("***obtenerParametrosInfoTriobutaria 8***");

		param.put(
				"MARCA_AGUA",
				obtenerMarcaAgua(infoTributaria.getAmbiente(), rechazado,
						dirServidor));
		log.info("***obtenerParametrosInfoTriobutaria 9***");
		param.put("NUM_FACT",
				infoTributaria.getEstab() + "-" + infoTributaria.getPtoEmi()
						+ "-" + infoTributaria.getSecuencial());
		log.info("***obtenerParametrosInfoTriobutaria 10***");
		param.put("AMBIENTE", obtenerAmbiente(infoTributaria));
		log.info("***obtenerParametrosInfoTriobutaria 11***");
		param.put("NOM_COMERCIAL", StringEscapeUtils.unescapeXml(infoTributaria
				.getNombreComercial()));
		
		log.info("***obtenerParametrosInfoTriobutaria 12***");
		return param;
	}

	private static String obtenerAmbiente(InfoTributaria infoTributaria) {
	...
	private static InputStream obtenerMarcaAgua(String ambiente,
			boolean rechazado, String dirServidor) {
		try {
			if (rechazado) {
	...
	private static Map<String, Object> obtenerInfoFactura(
			Factura.InfoFactura infoFactura, FacturaReporte fact) {

		log.info("***obtenerInfoFactura ***");
		Map<String, Object> param = new HashMap<String, Object>();
		param.put("DIR_SUCURSAL", StringEscapeUtils.unescapeXml(infoFactura.getDirEstablecimiento()));
		...
		param.put("RS_COMPRADOR", StringEscapeUtils.unescapeXml(infoFactura.getRazonSocialComprador()));
		...
		log.info("***if ***"+!tc.getIva12().equals("0.00"));
		...
		System.out.println("enviando nuevo parametro 1111");
		if(infoFactura.getCompensaciones() != null && infoFactura.getCompensaciones().getCompensacion() != null && !infoFactura.getCompensaciones().getCompensacion().isEmpty()){			
			...
			System.out.println("enviando nuevo parametro 2222"+compensacion+"---"+infoFactura.getImporteTotal());
		...
		log.info("***obtenerInfoFactura 222***");
		return param;
	}
	...
	private static TotalComprobante getTotalesNC(
			NotaCredito.InfoNotaCredito infoNc) {
		BigDecimal totalIva12 = new BigDecimal(0.0D);
		BigDecimal totalIva0 = new BigDecimal(0.0D);
		BigDecimal iva12 = new BigDecimal(0.0D);
		BigDecimal totalExentoIVA = new BigDecimal(0.0D);
	...
	private static TotalComprobante getTotalesND(
	...
	private static Map<String, Object> obtenerInfoNC(InfoNotaCredito infoNC,
...
//		param.put("IVA", tc.getIva12());
	...
	private static Map<String, Object> obtenerInfoGR(InfoGuiaRemision igr,GuiaRemision guiaRemision) {
		Map<String, Object> param = new HashMap<String, Object>();
		param.put("DIR_SUCURSAL",StringEscapeUtils.unescapeXml(igr.getDirEstablecimiento()));
		...
		param.put("RS_TRANSPORTISTA", StringEscapeUtils.unescapeXml(igr.getRazonSocialTransportista()));
	...
	public static List<InformacionAdicional> getInfoAdicional(GuiaRemision guiaRemision) {
	...
	private static String obtenerTotalDescuento(NotaCreditoReporte nc) {
		BigDecimal descuento = new BigDecimal(0);
		for (DetallesAdicionalesReporte detalle : nc.getDetallesAdiciones()) {
			descuento = descuento.add(new BigDecimal(detalle.getDescuento()!=null?detalle.getDescuento():"0"));
		}
		DecimalFormatSymbols otherSymbols = new DecimalFormatSymbols(
				Locale.getDefault());
		otherSymbols.setDecimalSeparator('.');
	...
	private static Map<String, Object> obtenerInfoND(
	...
	private static Map<String, Object> obtenerInfoCompRetencion(
	...
	public static Long obtenerPorcentajeIva(String fechaEmision)  {
