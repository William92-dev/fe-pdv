...
@MessageDriven(activationConfig = {
		...
		@ActivationConfigProperty(propertyName = "destination", propertyValue = "queue/guiaProcesarQueue"),
...
public class GuiaProcesarManagment implements MessageListener {

	@EJB
	private GuiaDAO servicioGuiaRemision;
	...
	private static Logger log = Logger.getLogger(GuiaProcesarManagment.class
			.getName());
			...
			if (mensaje != null && !mensaje.isEmpty() && mensaje.contains("&")) {
				...
				try {
					...
					Long tipo = servicioTipoComprobanteLocal.obtenerTipo(conn,
							ComprobanteEnum.GUIA_REMISION.getDescripcion());

					GuiaProcesarRequest item = servicioGuiaRemision.listar(
							conn, documentoVenta, farmacia, tipo);
					if (item != null) {
						...
						try {
							item.setSid(sid);
							//response = guia.procesar(item);
							response =  guia.procesarOffLine(item);
