...
@MessageDriven(activationConfig = {
		...
		@ActivationConfigProperty(propertyName = "destination", propertyValue = "queue/facturaProcesarQueue"),
...
public class FacturaProcesarManagment implements MessageListener {
	...
	private static Logger log = Logger.getLogger(FacturaProcesarManagment.class.getName());
	...
	public void onMessage(final Message message) {
		...
		try {
			...
			if (mensaje != null && !mensaje.isEmpty() && mensaje.contains("&")) {
				...
				log.info("***prueba compensacion 8888***");
				...
				try {
					...
					log.info("***prueba compensacion 8877***"+tipo);
					
					if (tipo == null) {
						tipo = 1L;
					}

					FacturaProcesarRequest item = servicioFactura.listar(conn,documentoVenta, farmacia, tipo);
					log.info("***333"+item.getFactura().getInfoTributaria().getClaveAcceso());
					if (item != null) {
						...
						try {
							item.setSid(sid);
							log.info("***prueba compensacion enviado a procesar***");
							//response = factura.procesar(item);
							response = factura.procesarOffline(item);

							log.info("********* Farmacia: " + farmacia
							...
							respuesta.setObservacion(response.getMensajeSistema());
