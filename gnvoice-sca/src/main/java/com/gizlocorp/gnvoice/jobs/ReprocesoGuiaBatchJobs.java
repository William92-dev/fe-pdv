...
@Stateless
public class ReprocesoGuiaBatchJobs {

	private static Logger log = Logger.getLogger(ReprocesoGuiaBatchJobs.class
			.getName());
	...
	public void inqueue(String mensaje) {
		...
		try {
			...
			MessageProducer producer = session
					.createProducer(this.queueReproceso);
	...
	private void scheduledTimeout(final Timer t) {
		try {
			...
			if (batch != null && "1".equals(batch)) {
				log.info("Ejecuta batch Guia de Remision... ");
				executebatch();
			} else {
				log.info("No ejecuta batch  Guia de Remision... ");
			}
		} catch (Exception e) {
			log.info("No ejecuta batch  Guia de Remision... ");
		}
